...
def song_cover_pipeline(zip_path, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            # 모든 파일 압축 해제
            zip_ref.extractall(root_output_dir)

        file_paths = get_file_paths(root_output_dir)
        ...
        for file_path in file_paths:
            ...
            ####### 추론 결과물 경로 생성
            ai_vocals_path = os.path.join(infer_output_dir, f'{file_name}.wav')

            ####### 추론
            if not os.path.exists(ai_vocals_path):
                display_progress('[~] Converting voice using RVC...', 0.5, is_webui, progress)
                voice_change(voice_model, file_path, ai_vocals_path, pitch_change, f0_method, index_rate, filter_radius, rms_mix_rate, protect, crepe_hop_length, is_webui)

        #####################  개별 폴더를 ZIP 파일로 압축
        display_progress('[~] Zipping the result folder...', 0.95, is_webui, progress)
        abs_root_output_dir = os.path.abspath(infer_output_dir)
        zip_filepath = abs_root_output_dir + '.zip'
        shutil.make_archive(abs_root_output_dir, 'zip', infer_output_dir)
